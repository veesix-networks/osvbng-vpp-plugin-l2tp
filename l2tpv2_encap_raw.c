...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  /* Make room for IP + UDP + L2TP-data + optional PPP HDLC
	   * Address/Control prefix. s->ppp_hdr_skip is 2 when the
	   * configured framing for this session/server is HDLC (the
	   * default, matches every major LNS — bngblaster, accel-ppp,
	   * Cisco, Nokia, RTBrick — and every pppd-based LAC), 0 when
	   * the operator opted into ACFC compressed framing.
	   *
